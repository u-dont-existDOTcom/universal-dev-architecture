...
> **Native Codex + concise owner/repository instructions + exact repository checks.**

No general workflow plugin earned a place in the default path.

- **Remove Superpowers.** Its engineering mode passed only one of four paired tasks, both Task C coordination variants failed, and removing it from maximum Task D saved 275 seconds and 547k input tokens. On real-project Task H it produced a correct preserved workspace but spent the full 1,200 seconds creating 11 workflow artifacts and making 24 unattributed waits without returning. The owner removed it during the audit.
- **Remove Engineering Guardrails.** Across paired Tasks A–F it produced no correctness win and uniquely failed Task E while claiming validation had been preserved. On Task H its workspace was correct but it timed out after 59 completed calls, 13 observed test commands, duplicate full-suite checks, and repeated reviews while native returned.
- **Leave Coordinator uncertain/conditional and outside the minimal stack.** Native and Coordinator both completed Tasks C and H correctly after adversarial review aligned Task H's hidden oracle with the prompt's recursive relocation contract. Coordinator cost more on C but was faster in the single H run (438 versus 1,073 agent-seconds). Its unique board never activated, so this mixed one-run-per-task result does not justify reinstalling or keeping it globally.
...
The frozen pre-removal snapshot contained 14 plugin manifests, 68 plugin skill files, 17 standalone/system skills, and two hook-bearing plugins. The final cache snapshot contains 10 manifests, 40 plugin skill files, the same 17 standalone/system skills, and no hook-bearing plugin. Process Jobs, Empire, Coordinator, Guardrails, and Security are absent from that cache. Superpowers remained as a cache artifact after the owner reported removing it; Visual Truth appeared during the audit and is classified unbenchmarked/install-on-demand.

The session-frozen tool schema contains 293 calls: 280 MCP tools and 13 native functions. GitHub (89), Linear (59), Nansen (38), Atlassian Rovo (31), Sites (22), and pre-removal Codex Security (19) are the largest named groups. Plugin removals may not alter a running session's schema, so this is not claimed as the next-session post-removal surface. A visible schema is not proof of account authorization or benefit.

The effective instruction chain includes unavoidable system/developer policy, `~/.codex/AGENTS.md`, repository `AGENTS.md`, `.github/AGENTS.md`, and `state/AGENTS.md`. The original global automatic-coordination agreement was constant in every condition, so its marginal effect was not measured. Its broad prescriptive preference for worktrees, durable ledgers, delegation, and independent review created a static risk of the same ceremony measured from optional workflow skills; the audit therefore replaced it with a proportional version that defaults small work to one agent and no durable artifact while retaining coordination authority. The owner also added a continuous-autonomous-completion agreement: continue routine work, ask only for material tradeoffs or missing authority, and prefer complete verified work over time-saving shortcuts without expanding into tangential work. Both edits occurred after the trials and are not credited with causal lift. The reversible-merge and GitHub keyring-boundary sections remain.
...
Tasks A-G are deterministic micro-projects (21-69 seed lines). They isolate causal behavior and withheld invariants, but they structurally favor low-ceremony execution. Task H challenged that bias with a real 146-file/~24k-line historical repository snapshot and four terminal workflow conditions under explicit owner authority. Task H uses one run per condition. Adversarial review found its trial-time hidden oracle had required a direct child of `excluded/` even though the prompt allowed intact relocation recursively under that directory; scorer schema 4 corrects the evaluator, preserves both trial-time and adjudicated hashes, and flags the change instead of silently rewriting provenance.

### Metrics and important limits

Correctness requires both visible and withheld tests. The structured scorer records whole-harness wall time, completed calls, actual completed test commands, CLI-reported tokens, changed non-generated files and text-line deltas (including untracked files), workflow artifacts, user-input gates, false completion claims, collaboration waits, and command failures. Generated bytecode/cache files are excluded. Partial measurements are labeled with coverage, and paired deltas become unavailable when either side lacks the metric.

The numeric **engineering-quality proxy** is deterministic bookkeeping based on test-file presence, changed-file count, workflow artifacts, and correctness. It is not a blinded assessment of architecture, simplicity, maintainability, or test design. Manual qualitative review therefore qualifies those process scores, caught a portability defect in Security's numerically passing patch, and caught generated-cache inflation in the first Task H scoring pass.
...
A terminal/UI crash killed one schedule supervisor while leaving its owned Superpowers child alive. The child was polled to the original 1,200-second boundary, the nonterminal attempt was preserved intact under `excluded/`, and the durable ledger rerun only that trial while skipping three terminal results. Earlier, a restarted sandbox made `~/.codex` read-only to nested Codex, so fifteen sub-second initialization attempts were also preserved and excluded before resuming at the host boundary.
...
| task-h | b0 r1 | yes | 77.0 | 100.0 | 1073.8 | 40 calls; 2333003 input tokens; 6 files; 1 workflow artifacts; 5 waits; 1 failed commands | 0 | — |
| task-h | coordinator r1 | yes | 90.0 | 100.0 | 463.4 | 25 calls; 827168 input tokens; 3 files; 1 failed commands | 0 | — |
| task-h | guardrails r1 | no (implementation correct; run failed) | 67.0 | 100.0 | 1201.0 | 59 calls; n/a input tokens; 6 files; 2 workflow artifacts; 7 waits; 3 failed commands | 0 | scored preserved workspace despite trial infrastructure failure; final message unavailable; token usage unavailable |
| task-h | superpowers-coordination r1 | no (implementation correct; run failed) | 45.0 | 100.0 | 1201.1 | 52 calls; n/a input tokens; 14 files; 11 workflow artifacts; 24 waits; 1 failed commands | 0 | scored preserved workspace despite trial infrastructure failure; final message unavailable; token usage unavailable |
...
Native B0 completed all seven A-G task classes correctly across eight trials, including two independent Task A runs, and native Tasks H and I also passed. It generally produced the smallest or tied-smallest diffs and lowest time, call, and token cost. B1 then showed that a concise repository contract can improve this baseline without a workflow plugin. On real-project Task H, B0 preserved incomplete evidence inside a collision-safe run-ID-containing reservation under `excluded/` and passed the full 112-test suite. The trial-time oracle rejected that valid nested layout; scorer schema 4 now accepts the stated recursive contract and records the evaluator change explicitly.
...
On Task H, Guardrails produced an implementation-correct preserved workspace. It still failed end to end at 1,200 seconds after 59 completed calls, 13 observed test commands, a failed branch-creation ceremony, duplicate full-suite execution, deterministic audit, and repeated review/fix cycles. Native delivered the same required correctness and returned, so Guardrails' extra process did not earn a place in the stack.
...
Task H reproduced the mechanism at real-project scale. Superpowers produced an implementation-correct preserved workspace, but spent its full 1,200-second budget on design/plan persistence, an SDD recovery directory, serialized task slices, per-slice review packages, and a final broad review. It made 24 unattributed waits, changed 14 non-generated files including 11 workflow artifacts, ran only two test commands visible to the parent event stream, and never reached terminal delivery. An earlier independent attempt was also still in review when the supervisor crash reached the same 1,200-second boundary. The owner removed Superpowers after the treatment had been frozen.

### Coordinator

Coordinator followed its own default-off contract because no marker or exact runtime task UUID existed. Its unique claim board therefore remains untested. On Task C, the treatment used more time, tokens, and calls than native with the same oracle outcome; its seven-file diff separated component tests that B0 kept in one larger file, so file count alone is not treated as an engineering-quality harm.

On Task H, Coordinator and native both passed the full 112-test suite and prompt-aligned recovery oracle. Coordinator returned in 438.4 agent-seconds with 827,168 input tokens and 25 calls; B0 returned in 1,073.4 agent-seconds with 2,333,003 input tokens and 40 calls. On Task C, the direction reversed: Coordinator added 24.1 seconds, 147,259 input tokens, and six calls for the same correctness. With one run per task and no active-board execution, this mixed evidence supports only `UNCERTAIN — CONDITIONAL / DO NOT INSTALL BY DEFAULT`. It is a future reevaluation candidate, not part of the smallest stack.

Removing Coordinator from maximum Task C did not change the outcome: both variants stopped for Superpowers approval with zero implementation. This shows no beneficial Coordinator/Superpowers interaction and reinforces the rule never to co-activate their task graphs.
...
| --- | --- | --- |
| Requirements, scope, and autonomy | Owner + repository instructions + native Codex | Continuous-autonomous-completion agreement usefully reinforces the owner's decision boundary; unablated policy, not plugin evidence |
...
| Decomposition and agents | Native parent with isolated worktrees and independent surfaces | Coordinator is outside the minimal stack; uncertain reevaluation only for substantial recovery-sensitive work, with active-board value still unmeasured |
...
| Persistence/recovery | Git, one execution plan, `state/CURRENT-STATE.md`, PR | Coordinator reevaluation only under a substantial recovery-sensitive trigger; claim-board value remains unmeasured |
...
| Coordinator shared-checkout lanes | Superpowers worktree/bite-size SDD | Conflict | Combined C stopped with no implementation | Never co-activate; Superpowers removed, Coordinator outside the minimal stack. |
| Guardrails/Superpowers completion gates | Owner continuous-autonomous-completion agreement | Conflict | Correct Task H workspaces failed to return after duplicate checks, 11 artifacts, and repeated reviews | Owner policy and one risk-proportionate verification owner are authoritative; remove both workflow plugins. |
...
| --- | --- | --- | --- | --- | --- |
| Engineering Guardrails | 5/7 end-to-end; 6/7 correct workspaces; median 184.1s, 287764 input tokens (6/7 measured), 23.0 calls | 8/8 end-to-end; 8/8 correct workspaces; median 107.7s, 158632 input tokens, 8.0 calls | Task H workspace was correct but timed out while native returned; no net correctness win and Task E regressed. | median deltas: +73.7s, n/a (incomplete) input tokens, +15.0 calls; 0 user gates, 2 workflow artifacts, 1 false completion claims | High |
...
| Coordinator | 2/2 end-to-end; 2/2 correct workspaces; median 322.8s, 589932 input tokens, 21.0 calls | 2/2 end-to-end; 2/2 correct workspaces; median 615.9s, 1269220 input tokens, 25.5 calls | Same correctness on C/H after prompt-aligned adjudication; added cost on C but one substantially faster H run. Unique board remained off, so marginal value is uncertain. | median deltas: -293.2s, -679288.0 input tokens, -4.5 calls; 0 user gates, 0 workflow artifacts, 0 false completion claims | Medium-low; one run per task |
| Superpowers coordination | 0/2 end-to-end; 1/2 correct workspaces; median 629.9s, 96772 input tokens (1/2 measured), 31.0 calls | 2/2 end-to-end; 2/2 correct workspaces; median 615.9s, 1269220 input tokens, 25.5 calls | Task H workspace was correct but timed out after repeated review gates while native returned; Task C was a zero-file failure. | median deltas: +14.0s, n/a (incomplete) input tokens, +5.5 calls; 1 user gates, 11 workflow artifacts, 0 false completion claims | High for default-path harm; same-task variance remains limited |
...
| Codex Security fix-finding skill | 1/2 end-to-end; 2/2 correct workspaces; median 747.4s, 1489711 input tokens (1/2 measured), 36.5 calls | 2/2 end-to-end; 2/2 correct workspaces; median 230.3s, 467695 input tokens, 15.0 calls | One additional TOCTOU idea on F, but no portable outcome lift; same-prompt I matched native. | median deltas: +517.0s, n/a (incomplete) input tokens, +21.5 calls; 0 user gates, 0 workflow artifacts, 0 false completion claims | High for default-path harm; medium for specialized analysis |
...
11. **GitHub workflow skills — REMOVE / REPLACE FROM DEFAULT PATH.** Use native `git`/`gh`; the connector itself was not ablated by these trials.

## Specialized and conditional components

Exact triggers are versioned in [activation-rules.md](../activation-rules.md).

- **Coordinator — UNCERTAIN / CONDITIONAL, DO NOT INSTALL BY DEFAULT:** one real Task H run was substantially faster than native, but Task C was slower, correctness matched after prompt-aligned adjudication, and the active board never ran. Preserve a narrow future reevaluation trigger; do not reinstall from this evidence or co-activate with another task graph.
...
- **Visual Truth:** uncertain/install-on-demand only for an explicit compatible React visual/WYSIWYG task; set `VISUAL_TRUTH_ANALYTICS=0` when install telemetry is not wanted.
...
- Global reversible-local-merge, continuous-autonomous-completion, and GitHub keyring-boundary instructions. Narrow the older automatic-coordination preference to proportional triggers.
...
- Coordinator only as an uncertain future reevaluation for substantial recovery-sensitive work; it is not part of the minimal installed stack and must never be combined with another task graph.
- Plugin Management.
- Exact domain apps/skills whose real work justifies them.

### Absent from the ordinary stack

Superpowers, Engineering Guardrails, GitHub workflow skills, Process Jobs, Empire, Default Templates, Exa, Parallels, unavailable browser recorder/performance skills, Coordinator, and the superseded broad automatic-coordination wording. Coordinator retains only an uncertain future reevaluation trigger. The GitHub connector, Visual Truth, and system-managed review agent are not ordinary-path components, and their narrow value remains uncertain.
...
- Condition `content_sha256` is a prompt-routing hash. Scorer schema 4 also stores and finalization enforces an `effective_surface_sha256` covering enabled skill paths/bodies and, when relevant, frozen plugin/app/hook stack plus runtime-tool evidence. Server-side connector drift outside the frozen schema remains possible.
...
- A-G are synthetic micro-projects. Task H adds one real 146-file/~24k-line repository task per selected workflow condition, but broad complex-project conclusions still need more real tasks and repetitions.
- Coordinator's unique durable shared-checkout claim-board mode was not staged across persistent windows. Mixed C/H cost evidence supports only uncertain conditional reevaluation, not installation, active-board value, or global activation.
- Task H's trial-time oracle over-specified direct-child relocation. The versioned adjudicated oracle accepts the prompt's recursive `under excluded/` contract; public records preserve both evaluator identities and flag all four affected trials.
...
- The proportional-coordination replacement, continuous-autonomous-completion agreement, and Visual Truth appeared after controlled trials; all are explicitly labeled unablated.
...
| --- | --- | ---: | --- | --- | --- | --- | ---: |
| Native Codex shell, Git, worktrees, subagents, and polling | KEEP | A-G B0 completed all seven task classes and eight baseline trials, and Tasks H and I also passed after the Task H oracle was aligned to the stated recursive relocation contract | Core implementation and execution substrate | Plugins wrap rather than replace it | No optional methodology context; one Task H run was slower than the Coordinator treatment | No development agent | 99% |
...
| Global automatic-coordination agreement | REPLACE WITH PROPORTIONAL VERSION — COMPLETED (UNABLATED) | The broad wording was constant in every controlled condition and therefore was not independently ablated; it was narrowed after the trials | Owner permission to choose coordination without an execution-mode question | Native judgment and repository plans already choose isolation and delegation | The former wording pressured worktrees, durable ledgers, delegation, and review without a proportionality threshold; the replacement defaults small work to one agent and no durable artifact | The completed replacement retains coordination authority while removing the broad prescriptive preference | 78% |
| Global continuous-autonomous-completion agreement | KEEP — OWNER POLICY (UNABLATED) | Added after the controlled trials at the owner's explicit request; not independently ablated and therefore not credited with benchmark lift | Durable owner preference to continue routine work, reserve questions for material tradeoffs, and favor complete verified outcomes over time-saving shortcuts | Reinforces native autonomy but supplies the owner's exact decision boundary | Small instruction cost; overreach is bounded by explicit scope, authority, and tangential-work exclusions | More obvious questions, premature stopping, and shortcut-biased execution may recur | 90% |
...
| Codex Engineering Guardrails 1.1.1 | REMOVE — HARMFUL | Five of six A-F paired runs passed versus seven of seven B0 runs; on real-project Task H its workspace was correct but it failed end to end at the 1200-second limit while native returned | No necessary capability beyond native implementation/review | High overlap with native reasoning, repository checks, and system review | Task H used 59 completed calls, 13 observed test commands, duplicate full-suite checks, two workflow artifacts, and never returned; Task E also regressed and made a false completion claim | Use native proportional planning, diagnosis, tests, and verification | 98% |
| Superpowers 6.2.0 | REMOVE — HARMFUL (OWNER REMOVED) | Engineering mode passed one of four A-E paired tasks, both Task C coordination variants failed, and real-project Task H produced a correct preserved workspace but failed to return within 1200 seconds | None needed; writing-skills is better covered by the system skill creator | Near-total overlap with native/repository planning, TDD, debugging, agents, review, and Git | Task H created 11 workflow artifacts, made 24 unattributed collaboration waits, serialized task and broad reviews, and timed out; earlier runs added three approval stops and 1.58M tokens/seven artifacts for a one-line fix | Lose prescriptive training-style rituals, not an execution primitive; the owner removed the plugin during the audit | 100% |
| Codex Coordinator 0.4.0 | UNCERTAIN — CONDITIONAL / DO NOT INSTALL BY DEFAULT | Coordinator and native both completed Tasks C and H correctly after the Task H oracle was aligned to the prompt. Coordinator was slower and costlier on small Task C but faster and cheaper in the single real Task H run: 438.4 versus 1073.4 agent-seconds, 827168 versus 2333003 input tokens, and 25 versus 40 calls | Active shared-checkout claim board remains unmeasured because exact runtime task IDs were unavailable and the skill correctly stayed default-off | Native subagents, isolated worktrees, Git, and repository recovery state cover ordinary work | Mixed one-run-per-task evidence, plugin-declared but unbenchmarked hooks, extra small-task cost, and an unbenchmarked active-board topology; the Task H speed result may be stochastic or instruction-treatment rather than board value | Ordinary work loses no measured correctness; an unverified shared-checkout claim registry and one unreplicated Task H speed result are lost | 64% |
...
| Visual Truth 1.3.0 | UNCERTAIN — INSTALL ON DEMAND | Installed during the audit after controlled trials; no outcome benchmark exists | Live visual selection, responsive measurement, WYSIWYG editing, and source-change briefs for compatible React applications | Native browser inspection and source editing partially overlap, but not the integrated live editor | Very large task instructions, application dependency/editor bridge, generated-source workflow, and a best-effort anonymous install event unless VISUAL_TRUTH_ANALYTICS=0; irrelevant outside explicit visual-editing tasks | Use native browser/source editing and manual responsive measurement | 45% |
