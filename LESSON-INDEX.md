...
28. `patterns/codex-pro-supervision-mission-control.md` — compose Symphony execution, Linear workflow state, GitHub task/code authority, deterministic evidence collection, an optional Extra High repository-evidence reader, task-bound Pro supervision, owner-decision gates, and an attention-first dashboard; keep retrieval separate from judgment, never require Pro web chats to access GitHub, distinguish progress/alignment/verification/confidence, and persist every packet, review, redirect, and decision by exact task/contract/evidence identity.
