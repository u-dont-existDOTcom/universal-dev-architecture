...
16. `patterns/existing-work-scan-and-scholarly-discovery.md` — before substantial bespoke method/framework/architecture work, preserve an independent conception when useful, search the underlying problem rather than only coined terminology, use domain-matched discovery tools (including scholarly semantic search such as SciSpace when academia is material), verify load-bearing claims against primary sources, classify solved/adaptable/incompatible/unresolved remainder, and choose reuse/adaptation/composition/invention/experiment explicitly.
