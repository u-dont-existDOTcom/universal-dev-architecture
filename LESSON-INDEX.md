...
21. `patterns/github-actions-pr-ref-namespace-safety.md` — when a `pull_request` workflow performs its own all-PR-head fetch after `actions/checkout`, keep audit PR heads in a namespace distinct from checkout-created `refs/remotes/pull/<n>/merge` refs (for example `refs/remotes/pull-heads/*`), and validate the exact ref topology in a real PR Actions run rather than assuming a local clone reproduces it.
