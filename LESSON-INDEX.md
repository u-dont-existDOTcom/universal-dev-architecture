...
40. `patterns/worker-directive-delivery-and-chat-output-budget.md` — when worker execution is selected as the next action, deliver the complete runnable directive in the same turn instead of stopping at meta-commentary; keep short directives in one fenced code block and materialize longer operational payloads as `.md`/text artifacts with concise chat summaries to reduce scrollback and output latency.
