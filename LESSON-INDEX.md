...
When a consequential task has an exclusive active-task lock, that branch-bound task identity and its preflight outrank unrelated handoffs, worktree checkpoints, autonomous roadmaps, and global next-task selection for that worker. Ordinary green tests remain prerequisites; the task-specific acceptance command defines readiness only for the boundary that acceptance command is designed to protect. If the owner explicitly changes the active goal back to rapid experimentation, update/supersede a stale release-grade task lock rather than treating it as permanently authoritative.
