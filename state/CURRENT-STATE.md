...
- Current step: 50 terminal records are normalized and sanitized; 49 current/equivalent records drive the report and one materially different historical maximum trial is excluded from current aggregation. Final release review is clear, PR #17 and the sanitized `codex-plugin-fix` lessons repository are published, and the focused full-history checkout repair passed every local gate after the first PR check exposed the shallow-clone dependency.
...
- Required completion: push the focused checkout repair to PR #17, confirm the replacement remote checks pass, update the companion provenance to the final audit head, and verify remote heads and links.
