...
export function cycleControlPrompt(route, step, { omitContinuationOwnerExactText = false } = {}) {
  ...
  if (route.packet.routeSchemaVersion === 6) {
    ...
    return [
      ...
      `Use only the selected ${github} app to read the bound evidence references. Do not write, comment, mutate, or otherwise use GitHub after those reads. Return exactly one canonical decision block in the final assistant message; a deterministic Mission Control copier will publish those exact machine bytes to ${binding.decision_receipt_target.immutable_issue_url}. Do not select, call, or require the ${missionControl} app or any MCP tool.`,
      ...
      'Return MISSION_CONTROL_CANONICAL_DECISION_V1 followed by a newline and strict JSON with schema_version 5, envelope_kind MISSION_CONTROL_CANONICAL_DECISION, request_id, supervisor_id, provider_session_id, nonce, in_band_binding_sha256, execution_provenance IN_BAND_REQUEST_BINDING_GITHUB_OBSERVED, evidence_capsule, owner_outcome, reasoning_lane, decision_block {decision_id,exact_text,sha256}, pro_decision_block, and writer_contract {mode:EXACT_COPY_OR_STRUCTURED_TRANSFORMATION_ONLY,reinterpretation_allowed:false}. The canonical block must be the final content in the response; a mandatory visible timestamp may precede it, but nothing may follow the JSON.',
      ...
      'Read only the bound evidence, reason only on the bounded request, return one canonical decision block, then stop. If any binding, evidence-read, hash, or expiry check fails, fail closed. Do not attempt a GitHub write, answer with substitute prose, delegate to Work, call paid model APIs, Continue, Retry, or create another provider request.',
    ].filter(Boolean).join('\n');
  }
...
function boundedExecutionResidueInstruction(route) {
  ...
  return ` If and only if the accepted decision leaves bounded local filesystem/command work or an already-qualified restricted-browser operation that Chat cannot execute, include one optional top-level bounded_execution object authored completely in this decision. It must use schema_version 1 and contain exactly task_id, job_id, execution_objective, reasoning_summary, strategy_id, strategy_causal_hypothesis, predicted_outcome_change, success_threshold, failure_threshold, next_decision_changing_evidence, reviewed_evidence_boundary, inputs, allowed_actions, allowed_paths, allowed_commands, forbidden_actions, forbidden_paths, forbidden_decisions, required_evidence, required_tests_or_checks, stop_and_return_triggers, maximum_execution_cycles, execution_capability, workspace, output_schema, prompt, deadline, work_execution_profile, optional execution_surface, and optional retry_of_attempt_id. Machine-type contract: task_id must be ${JSON.stringify(taskId)}; job_id and retry_of_attempt_id must match ^[A-Za-z0-9][A-Za-z0-9._-]*$ (colon is forbidden); reviewed_evidence_boundary must be one non-empty STRING, never an array; inputs must be a non-empty JSON ARRAY of strict objects {"type":<non-empty string>,"ref":<non-empty string>,"sha256":<64-lowercase-hex or null>}; allowed_actions, allowed_paths, allowed_commands, forbidden_actions, forbidden_decisions, required_evidence, required_tests_or_checks, and stop_and_return_triggers must each be non-empty arrays of non-empty strings; forbidden_paths must be an array of strings; maximum_execution_cycles must be a positive integer; workspace must be one absolute path STRING beginning with /, never an object; output_schema must be a JSON object; deadline must be an offset-aware ISO timestamp. execution_capability must be either {"type":"LOCAL_FILESYSTEM_COMMAND"} or {"type":"BROWSER","name":"EXAMPLE_TARGET_LIFECYCLE"}; another browser name deliberately routes to the existing exact legacy path. work_execution_profile is STRICT and must contain only model, effort, routingTier, routingTriggers, fastModeRequest, assuranceRequirement, policyRef, routingPolicyBaseCommit, contractVersion. Allowed model values: GPT_5_6_SOL or GPT_6_ASTRA. Allowed effort values: LOW, MEDIUM, HIGH, XHIGH, MAX. Allowed routingTier values: SOL_LOW, SOL_MEDIUM, SOL_HIGH_EXCEPTION, ASTRA_LOW, ASTRA_MEDIUM, ASTRA_HIGH, ASTRA_XHIGH, ASTRA_MAX; model/effort must match the tier. routingTriggers is an array of machine-code strings matching ^[A-Z0-9][A-Z0-9_:.\/-]*$. fastModeRequest is DO_NOT_ENABLE_FAST or ENABLE_FAST. assuranceRequirement is SET_REQUEST_SUFFICIENT or INDEPENDENT_READBACK_REQUIRED. policyRef must equal "patterns/work-model-and-effort-routing.md", routingPolicyBaseCommit must equal "fc3d0d7592a4fa69e94ff8ae31d9a4e5433b73cb", and contractVersion must equal "TRUSTED_SETTER_V1". For ordinary bounded Sol Medium work with no stronger source-bound routing trigger, use exactly {"model":"GPT_5_6_SOL","effort":"MEDIUM","routingTier":"SOL_MEDIUM","routingTriggers":[],"fastModeRequest":"DO_NOT_ENABLE_FAST","assuranceRequirement":"SET_REQUEST_SUFFICIENT","policyRef":"patterns/work-model-and-effort-routing.md","routingPolicyBaseCommit":"fc3d0d7592a4fa69e94ff8ae31d9a4e5433b73cb","contractVersion":"TRUSTED_SETTER_V1"}. If the bounded request explicitly requires native ChatGPT Work, set execution_surface to "CHATGPT_WORK_CLOUD"; otherwise omit it or use "CODEX". Author every semantic field now; if any required field is unavailable, omit bounded_execution entirely. Work must never fill, infer, or upgrade missing residue; it must never repair missing residue either.`;
}
