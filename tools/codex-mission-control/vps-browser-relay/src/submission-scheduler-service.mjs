...
const OPEN_QUEUE_STATUSES = new Set(['QUEUED', 'PRECLICK_RETRY_PENDING', 'ADMITTED']);
...
export class SchedulerStateStore {
  constructor({ stateFile, lockFile = `${stateFile}.lock` }) {
  ...
  async acquireLock() {
    ...
    try {
      ...
      if (error?.code === 'EEXIST') {
        if (!await this.#lockOwnerIsProvablyDead()) throw new Error(`Another submission scheduler owns ${this.lockFile}.`);
        await unlink(this.lockFile);
        this.lockHandle = await open(this.lockFile, 'wx', 0o600);
        await this.lockHandle.writeFile(`${JSON.stringify({ pid: process.pid, acquiredAt: new Date().toISOString(), replacedProvablyDeadPid: true })}\n`);
        return;
      }
      throw error;
    }
  ...
  async read() {
    try {
      return normalizeSchedulerState(JSON.parse(await readFile(this.stateFile, 'utf8')));
    } catch (error) {
      if (error?.code === 'ENOENT') return defaultSchedulerState();
      if (error instanceof SyntaxError) throw new Error(`Submission scheduler state is invalid JSON: ${this.stateFile}`);
      throw error;
    }
  }

  async write(value) {
    const state = normalizeSchedulerState(value);
  ...
  async #lockOwnerIsProvablyDead() {
    try {
      const parsed = JSON.parse(await readFile(this.lockFile, 'utf8'));
      if (!Number.isInteger(parsed.pid) || parsed.pid < 1) return false;
      try {
        await access(`/proc/${parsed.pid}`);
        return false;
      } catch (error) {
...
export class CentralSubmissionScheduler {
  ...
  async admit(raw, producerId) {
    return this.#serialized(async () => {
      ...
      request = resolveRetryRequest(state, request, providerRateLimitCount);
      chat = assertRegistryTarget(this.chats, state, request);
  ...
  async abortBeforeBoundary(raw, producerId) {
    return this.#serialized(async () => {
      ...
      const count = rateLimitCount(state, admission.retryRootKey);
      queueItem.status = count >= 2 ? 'RATE_LIMIT_RETRY_EXHAUSTED' : 'PRECLICK_RETRY_PENDING';
      queueItem.lastPreclickAbortAt = admission.abortedAt;
      if (count >= 2) queueItem.terminalAt = admission.abortedAt;
      await this.stateStore.write(state);
...
export async function startSubmissionSchedulerService({ config, scheduler, stateStore, logger = console }) {
  await stateStore.acquireLock();
  await scheduler.activateLease(config.lease);
  const server = http.createServer(async (request, response) => {
    try {
      const url = new URL(request.url ?? '/', `http://${request.headers.host ?? `${config.host}:${config.port}`}`);
      if (request.method === 'GET' && url.pathname === '/health') return json(response, 200, { status: 'ok' });
      authorize(request, config.token);
      if (request.method === 'GET' && url.pathname === '/status') return json(response, 200, await scheduler.status());
      ...
      if (typeof producerId !== 'string' || !/^[A-Za-z0-9][A-Za-z0-9._:/-]{0,179}$/.test(producerId)) {
        throw new SubmissionSchedulerError('PRODUCER_ID_INVALID', 'A valid relay producer identity is required.', 403);
      }
      if (request.method === 'POST' && url.pathname === '/admissions') return json(response, 201, await scheduler.admit(await readJson(request), producerId));
      if (request.method === 'POST' && url.pathname === '/admissions/validate') return json(response, 200, await scheduler.validateAdmission(await readJson(request), producerId));
      if (request.method === 'POST' && url.pathname === '/boundaries') return json(response, 201, await scheduler.recordBoundary(await readJson(request), producerId));
      if (request.method === 'POST' && url.pathname === '/target-bindings') return json(response, 201, await scheduler.bindTarget(await readJson(request), producerId));
      if (request.method === 'POST' && url.pathname === '/provider-rate-limits') return json(response, 201, await scheduler.recordRateLimit(await readJson(request), producerId));
      if (request.method === 'POST' && url.pathname === '/aborts') return json(response, 200, await scheduler.abortBeforeBoundary(await readJson(request), producerId));
      return json(response, 404, { error: 'Not found' });
    } catch (error) {
      if (error instanceof SubmissionSchedulerError) return json(response, error.statusCode, { error: error.message, code: error.code, ...error.detail });
      logger.error(error);
      return json(response, 500, { error: 'Submission scheduler request failed.' });
    }
  });
...
function validateQueueAdmissionCorrespondence(item, linked) {
  ...
  if (item.status === 'RATE_LIMIT_RETRY_EXHAUSTED'
    && (latest?.status !== 'ABORTED_BEFORE_BOUNDARY' || !latest.providerRateLimitObservedAt)) {
...
function schedulerStatus(state, minIntervalMs, nowMs) {
  ...
  return {
    ...
    lastSubmissionAt: state.lastBoundaryAt,
    retryAfterMs,
    nextSubmissionAt: Number.isFinite(lastMs) ? new Date(lastMs + minIntervalMs).toISOString() : null,
    ready: Boolean(leaseReady && retryAfterMs === 0 && !open && !state.safetyHalt),
...
function resolveRetryRequest(state, request, count) {
  if (count !== 1 || request.queueKey !== request.retryRootKey) return request;
  const crossedRateLimitedRoot = state.admissions.some((item) => item.retryRootKey === request.retryRootKey
    && item.queueKey === request.retryRootKey && item.status === 'BOUNDARY_RECORDED' && item.providerRateLimitObservedAt);
  if (!crossedRateLimitedRoot) return request;
  return { ...request, queueKey: `${request.retryRootKey}:provider-rate-limit-retry:1` };
}
...
function authorize(request, expected) {
  ...
  if (left.length !== right.length || !timingSafeEqual(left, right)) throw new SubmissionSchedulerError('SCHEDULER_UNAUTHORIZED', 'Invalid scheduler credential.', 401);
}
...
async function readJson(request) {
  ...
  for await (const chunk of request) {
    ...
    size += buffer.length;
    if (size > 100_000) throw new SubmissionSchedulerError('SCHEDULER_BODY_TOO_LARGE', 'Request exceeds 100000 bytes.', 413);
    chunks.push(buffer);
  }
  try { return JSON.parse(Buffer.concat(chunks).toString('utf8')); }
  catch { throw new SubmissionSchedulerError('SCHEDULER_JSON_INVALID', 'Request body must be JSON.', 400); }
}
