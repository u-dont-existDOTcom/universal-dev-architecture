...
function buildDirectWorkPrompt(input: {
  ...
  return [
    ...
    `On COMPLETED, PARTIAL, BLOCKED, or FAILED, write exactly one GitHub issue comment to ${receiptTarget}.`,
    `Prefix the comment with ${WORK_CLOUD_EXECUTION_RECEIPT_PREFIX.trimEnd()} followed immediately by one strict JSON object.`,
