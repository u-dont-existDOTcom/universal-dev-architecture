...
export class CentralSubmissionScheduler {
  ...
  // Called only by protected startup configuration, never by the relay dispatcher.
  async recoverBeforeComposition(rawPermit, configuredLease, producer) {
    return this.#serialized(() => {
      const permit = parsePrecompositionRecoveryPermit(rawPermit);
      if (permit.pacingDomain !== this.pacingDomain) throw recoveryError('SCOPE_MISMATCH');
      if (producer?.kind !== 'COLLECTOR' || producer.id !== permit.producerId
        || !Array.isArray(producer.workerScopes) || !Array.isArray(producer.taskScopes)) throw recoveryError('PRODUCER_MISMATCH');
      if (typeof this.stateStore.recoverBeforeComposition !== 'function') throw recoveryError('TRANSACTION_UNAVAILABLE');
      return this.stateStore.recoverBeforeComposition(permit, (state) => {
        const nowMs = this.now();
        if (Date.parse(permit.issuedAt) > nowMs || Date.parse(permit.expiresAt) <= nowMs) throw recoveryError('PERMIT_EXPIRED');
        if (!sameLease(state.activeLease, parseDeploymentLease(configuredLease))
          || recoverySha256(state.activeLease) !== permit.expectedLeaseSha256) throw recoveryError('LEASE_MISMATCH');
        const admission = state.admissions.find((item) => item.admissionId === permit.admissionId);
        const queueItem = state.queueItems.find((item) => item.queueItemId === permit.queueItemId);
        if (!admission || !queueItem || admission.status !== 'AMBIGUOUS_AFTER_RESTART'
          || queueItem.status !== 'AMBIGUOUS_AFTER_RESTART'
          || queueItem.admissionIds.at(-1) !== admission.admissionId
          || admission.queueItemId !== queueItem.queueItemId) throw recoveryError('RESERVATION_MISMATCH');
        if (recoverySha256(admission) !== permit.expectedAdmissionSha256
          || recoverySha256(queueItem) !== permit.expectedQueueItemSha256
          || queueItem.requestFingerprint !== permit.requestFingerprint
          || queueItem.logicalFingerprint !== permit.logicalFingerprint
          || admission.requestId !== permit.requestId || admission.producerId !== permit.producerId
          || admission.sendPath !== permit.sendPath || admission.bodySha256 !== permit.bodySha256) throw recoveryError('RESERVATION_MISMATCH');
        assertActiveLease(state.activeLease, admission, nowMs);
        const binding = state.relayBindings[permit.producerId];
        if (!this.producerBindings.has(permit.producerId)
          || recoverySha256(binding ?? null) !== permit.expectedRelayBindingSha256) throw recoveryError('BINDING_MISMATCH');
        assertProducerBinding(state.relayBindings, permit.producerId, admission);
        const configuredBinding = this.producerBindings.get(permit.producerId);
        if (configuredBinding.hostAlias !== binding.hostAlias || configuredBinding.hostRole !== binding.hostRole) throw recoveryError('BINDING_MISMATCH');
        initializeOrValidateRelayBindings(state, this.producerBindings);
        const chat = assertRegistryTarget(this.chats, state, admission);
        const authorizationRef = chat.workerId ? `task:${chat.workerId}` : 'task:mission-control';
        if ((chat.workerId && !producer.workerScopes.includes('*') && !producer.workerScopes.includes(chat.workerId))
          || admission.authorizationRef !== authorizationRef
          || (!producer.taskScopes.includes('*') && !producer.taskScopes.includes(authorizationRef))) throw recoveryError('PRODUCER_MISMATCH');
        if (admission.targetKind !== 'REGISTERED_BOOTSTRAP') throw recoveryError('SCOPE_MISMATCH');
        if (state.safetyHalt || state.relayTargetTransition) throw recoveryError('SAFETY_GATE_CLOSED');
        if (state.admissions.some((item) => item.admissionId !== admission.admissionId
          && (item.status === 'ADMITTED' || item.status.startsWith('AMBIGUOUS')))
          || state.queueItems.some((item) => item.queueItemId !== queueItem.queueItemId
            && (item.status === 'ADMITTED' || item.status.startsWith('AMBIGUOUS')))) throw recoveryError('OTHER_AMBIGUITY');
        if (state.lastBoundaryAt !== admission.previousGlobalBoundaryAt
          || state.admissions.some((item) => item.queueItemId === queueItem.queueItemId
            && (hasSubmissionEvidence(item) || item.providerRateLimitObservedAt))
          || hasSubmissionEvidence(admission) || admission.abortedAt || admission.abortStage) throw recoveryError('BOUNDARY_OR_OUTCOME_PRESENT');
        const failedMs = Date.parse(permit.failureEvidence.observedAt);
        if (failedMs < Date.parse(admission.admittedAt) || failedMs > Date.parse(permit.issuedAt)
          || Date.parse(admission.expiresAt) > nowMs) throw recoveryError('EVIDENCE_TIME_MISMATCH');
        admission.status = PRECOMPOSITION_RECOVERED;
        admission.precompositionRecovery = { permit, permitSha256: recoverySha256(permit), recoveredAt: new Date(nowMs).toISOString() };
