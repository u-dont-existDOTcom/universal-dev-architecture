...
export class CentralSubmissionScheduler {
  ...
  async activateLease(rawLease, { restorePersisted = false } = {}) {
    return this.#serialized(async () => {
      ...
      // A restart must not roll a newer persisted lease back to static boot
      // configuration. Restoring a stale lease permits inspection and a validated
      // successor transition, not sends or renewal of the expired epoch.
