...
export interface PublicMcpDependencies {
  ...
  now?: () => string;
  recordAccess?: (event: PublicMcpAccessEvent) => void;
}
...
export function createPublicMissionControlMcpServer(dependencies: PublicMcpDependencies) {
  ...
  server.registerTool(publicMcpToolNames[0], {
    ...
    try {
      ...
      if (!result || result.chat_id !== chat_id) {
        access(dependencies, { tool: publicMcpToolNames[0], challenge_id, chat_id, status: "NOT_FOUND", occurred_at: now });
        throw notFound();
      }
      access(dependencies, { tool: publicMcpToolNames[0], challenge_id, chat_id, status: "OK", occurred_at: now });
      return structuredResult(result, "The exact current capability challenge metadata is in structuredContent.");
    } catch (error) {
      if (error instanceof McpError) throw error;
      access(dependencies, { tool: publicMcpToolNames[0], challenge_id, chat_id, status: "UNAVAILABLE", occurred_at: now });
      throw unavailable();
    }
  });
  ...
  server.registerTool(publicMcpToolNames[1], {
    ...
    description: "Read the exact current admitted control-plane binding for one pending supervisory request and registered chat. This tool exposes no evidence body, transcript, owner message, or private worker state.",
    inputSchema: {
      request_id: exactId("Exact pending Mission Control supervisory request ID."),
      chat_id: exactId("Exact registered Mission Control supervisor chat ID."),
    },
    outputSchema: {
      schema_version: z.literal(1), request_id: z.string(), request_nonce: z.string(), chat_id: z.string(),
    ...
    annotations: readOnlyAnnotations,
    _meta: noAuthMeta,
  }, async ({ request_id, chat_id }) => {
    const now = currentTime(dependencies);
    try {
      const result = publicSupervisoryRequestBinding(await dependencies.loadEvents(), dependencies.loadPolicy(), request_id, chat_id, now);
      if (!result) {
        access(dependencies, { tool: publicMcpToolNames[1], request_id, chat_id, status: "NOT_FOUND", occurred_at: now });
        throw notFound();
      }
      access(dependencies, { tool: publicMcpToolNames[1], request_id, chat_id, status: "OK", occurred_at: now });
      return structuredResult(result, "The exact current supervisory request binding is in structuredContent.");
    } catch (error) {
      if (error instanceof McpError) throw error;
      access(dependencies, { tool: publicMcpToolNames[1], request_id, chat_id, status: "UNAVAILABLE", occurred_at: now });
      throw unavailable();
    }
  });
  ...
  server.registerTool(publicMcpToolNames[2], {
    ...
    description: "Read receipt IDs, statuses, timestamps, and CONTINUE_REQUIRED count for the exact current escalated supervisory request/chat. This metadata has no semantic authority and contains no decision or assistant content.",
    inputSchema: {
      request_id: exactId("Exact pending Mission Control supervisory request ID."),
      chat_id: exactId("Exact registered Mission Control supervisor chat ID."),
    },
    outputSchema: {
      schema_version: z.literal(1), request_id: z.string(), chat_id: z.string(),
    ...
    try {
      ...
      const binding = publicSupervisoryRequestBinding(events, dependencies.loadPolicy(), request_id, chat_id, now);
      const result = binding?.reasoning_lane === "PRO_ESCALATED"
        ? publicStageLivenessState(events, request_id, chat_id, binding.queued_at, binding.expires_at, now)
        : null;
      if (!result) {
        access(dependencies, { tool: publicMcpToolNames[2], request_id, chat_id, status: "NOT_FOUND", occurred_at: now });
        throw notFound();
      }
      access(dependencies, { tool: publicMcpToolNames[2], request_id, chat_id, status: "OK", occurred_at: now });
      ...
      if (error instanceof McpError) throw error;
      access(dependencies, { tool: publicMcpToolNames[2], request_id, chat_id, status: "UNAVAILABLE", occurred_at: now });
      throw unavailable();
    }
  });
...
export function publicSupervisoryRequestBinding(
  ...
  const matches = pendingDecisionRequests(events).filter((request) => request.requestId === requestId && request.chatId === chatId);
...
export function publicStageLivenessState(
  ...
  for (const event of [...events].sort((left, right) => left.sequence - right.sequence)) {
    ...
    if (!event.data.refs.includes(`request:${requestId}`) || !event.data.refs.includes(`chat:${chatId}`)) continue;
...
function access(dependencies: PublicMcpDependencies, event: Omit<PublicMcpAccessEvent, "event">) {
  dependencies.recordAccess?.({ event: "mission_control_public_mcp_tool_call", ...event });
}
