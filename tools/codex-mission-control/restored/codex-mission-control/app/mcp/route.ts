...
async function recordBindingTransportAttempt(request: Request) {
  ...
  const pending = pendingDecisionRequests(events).find((candidate) => (candidate.routeSchemaVersion === 3 || candidate.routeSchemaVersion === 4 || candidate.routeSchemaVersion === 5)
