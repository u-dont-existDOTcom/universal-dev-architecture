...
const chatId = "mc-hotfix-specialist";
...
test("request binding returns only exact current admitted control metadata and rejects wrong, stale, completed, or superseded bindings", async () => {
  ...
  const result = await mcpTool(dependencies({ events }), "get_supervisory_request_binding", { request_id: requestId, chat_id: chatId });
  assert.deepEqual(Object.keys(result.structuredContent), [
    "schema_version", "request_id", "request_nonce", "chat_id", "worker_id", "reasoning_lane", "queued_at", "expires_at",
  ...
  await assertToolFailure(dependencies({ events }), "get_supervisory_request_binding", { request_id: requestId, chat_id: "wrong-chat" });
  await assertToolFailure(dependencies({ events }), "get_supervisory_request_binding", { request_id: "unknown", chat_id: chatId });
  await assertToolFailure(dependencies({ events, currentTime: "2026-09-05T00:00:00.000Z" }), "get_supervisory_request_binding", { request_id: requestId, chat_id: chatId });
  await assertToolFailure(dependencies({ events: [...events, completedDecisionEvent()] }), "get_supervisory_request_binding", { request_id: requestId, chat_id: chatId });
  const superseded = structuredClone(events);
  superseded.push(ownerOutcomeEvent(99, "different-owner-outcome"));
  await assertToolFailure(dependencies({ events: superseded }), "get_supervisory_request_binding", { request_id: requestId, chat_id: chatId });
});
...
test("stage state contains only non-semantic receipt metadata and rejects non-current or non-escalated requests", async () => {
  ...
  const result = await mcpTool(dependencies({ events }), "get_stage_liveness_state", { request_id: requestId, chat_id: chatId });
  assert.deepEqual(Object.keys(result.structuredContent), [
    "schema_version", "request_id", "chat_id", "extra_high_reader", "pro_reasoner", "extra_high_reader_continue_required_count",
  ...
  await assertToolFailure(dependencies({ events }), "get_stage_liveness_state", { request_id: requestId, chat_id: "wrong-chat" });
  const direct = currentEvents("EXTRA_HIGH_DIRECT");
  await assertToolFailure(dependencies({ events: direct }), "get_stage_liveness_state", { request_id: requestId, chat_id: chatId });
});

test("pure projections fail closed and never surface unrelated event content", () => {
  const events = [...currentEvents(), privateBodyEvent(7)];
  const binding = publicSupervisoryRequestBinding(events, policy(), requestId, chatId, now);
  assert.ok(binding);
  assert.equal(Object.hasOwn(binding, "factual_packet"), false);
  const liveness = publicStageLivenessState(events, requestId, chatId, "2026-09-03T18:00:00.000Z", expiry, now);
  assert.ok(liveness);
...
function policy(): GitHubReceiptPolicy {
  return {
    ...
    capabilityChallenges: [{
      challengeId: "challenge-safe-1", chatId, worker: workerId, mcNonce: "disposable-mc-nonce", githubNonce: "disposable-github-nonce",
...
function currentEvents(lane: "EXTRA_HIGH_DIRECT" | "PRO_ESCALATED" = "PRO_ESCALATED"): StoredEvent[] {
  return [ownerOutcomeEvent(1, "owner-outcome-safe"), requestEvent(2, lane)];
}
...
function requestEvent(sequence: number, lane: "EXTRA_HIGH_DIRECT" | "PRO_ESCALATED"): StoredEvent {
  const body = supervisoryCycleRoutePrefix + JSON.stringify({
    schemaVersion: 2, packetKind: "SAME_CHAT_SUPERVISORY_CYCLE", requestId, destinationChatId: chatId,
...
function stageEvent(sequence: number, receiptId: string, stage: "EXTRA_HIGH_READER" | "PRO_REASONER", status: "STAGE_COMPLETE" | "CONTINUE_REQUIRED", occurredAt: string): StoredEvent {
  return event(sequence, {
    ...
    summary: stageLivenessSummary, refs: [`request:${requestId}`, `chat:${chatId}`, `stage:${stage}`, `status:${status}`, "semantic_authority:false"],
