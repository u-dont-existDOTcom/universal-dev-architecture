...
Before Codex or Work authors a proposal, methodology, priority, spending design, consequential tradeoff, external execution plan, or other Chat-owned semantic decision, it must call the authenticated runtime admission endpoint through `npm run supervision:admit:runtime -- --worker <id> --input <request.json>`. No work command may follow unless the returned `mayExecute` value is exactly `true` and the response binds a verified Chat-authored source receipt to a bounded zero-spend execution directive.
