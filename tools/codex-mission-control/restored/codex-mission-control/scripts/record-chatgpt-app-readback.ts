...
async function main() {
  ...
  const conversationUrl = arg("--conversation-url"), promptSha256 = digest(arg("--prompt-sha256"), "prompt"), machineBlockSha256 = digest(arg("--machine-block-sha256"), "machine block"), threadIdSha256 = digest(arg("--thread-id-sha256"), "thread id"), observedAt = arg("--observed-at");
  ...
  const refs = [`request:${requestId}`, `supervisor:${supervisorId}`, `provider_session:${providerSessionId}`, "status:COMPLETE", `machine_block_sha256:${machineBlockSha256}`, `provider_prompt_sha256:${promptSha256}`, `conversation_url:${conversationUrl}`, "thread_surface:chatgpt", `app_thread_id_sha256:${threadIdSha256}`, "semantic_authority:false", "readback_method:APP_OWNED_THREAD_EXACT_MACHINE_BLOCK"];
  const identity = { worker, taskId, requestId, supervisorId, providerSessionId, conversationUrl, promptSha256, machineBlockSha256, threadIdSha256, observedAt };
