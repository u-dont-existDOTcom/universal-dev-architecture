# Current research-worker supervision — long-range research addendum
...
- `../patterns/work-model-and-effort-routing.md`; and
...
- owner or reasoning decision needed, if any.

An interim snapshot must be labeled nonterminal and state what work remains.

## Routing

Use current routing authority rather than hard-coded model names.

- Chat/reasoning owns research questions, methodology, synthesis, adequacy, strategy, scope changes, and owner interaction.
- Execution workers handle only bounded implementation/acquisition/parsing/calculation/schema/test work from a current directive.
- Apply `../patterns/reasoning-selection.md` for reasoning method/effort and `../patterns/work-model-and-effort-routing.md` only when Work is actually admitted.
- Escalate to the strongest currently authorized reasoning surface only when a consequential unresolved judgment warrants it.

A reasoning surface advises; it does not become the mission controller or evidence authority.
