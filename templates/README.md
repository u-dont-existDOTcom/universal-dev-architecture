...
- `REASONING-SURFACE-OBSERVATION-RECEIPT.json` — single-transaction signed-in UI observation binding an externally required non-anonymous account, exact visible mode, session, evaluator-owned declarative input transform, admission question, completed response, strict-RFC3339 per-observation evidence, post-response mode, and replay protection without claiming platform attestation.
- `SUPERVISION-VERDICT-ADMISSION.json` — single-use account-, admission-question-, and response-digest binding between a reasoning-surface receipt and an admitted supervisory verdict.
