...
- `CLAIM-TRANSITION.json` — append-only `DERIVED` / `PROMOTED` / `REVOKED` / `SUPERSEDED` transition ledger entry with validated prior-record digest chaining and exact claim-ref continuity.
- `CLAIM-REPRODUCTION-RECEIPT.json` — exact-subject independent reproduction with runtime-validated independence evidence that can verify a fact but can never promote policy authority.
- `REASONING-SURFACE-OBSERVATION-RECEIPT.json` — single-transaction signed-in UI observation binding account, exact visible mode, session, executable input-to-submission transform, admission question, completed response, per-observation evidence/time, post-response mode, and replay protection without claiming platform attestation.
- `SUPERVISION-VERDICT-ADMISSION.json` — single-use admission-question- and response-digest binding between a reasoning-surface receipt and an admitted supervisory verdict.
- `RECEIPT-CONSUMPTION-EVENT.json` — append-only durable ledger event binding admission question and response while preventing receipt/nonce/transaction replay across process restarts.
