...
- `AUTHORITY-SOURCE-REGISTRY.json` — relying-party-supplied immutable authority sources with exact record fields plus per-source and whole-registry digests; runtime inputs are exact-type checked and completely reconstructed at every use so embedded declarations, partial records, or helper-minted objects cannot self-authorize.
- `CLAIM-RECORD.json` — versioned, subject-bound claim with conjunctive required authorizations, strict-RFC3339 creation/optional expiry times, and use-site-derived load-bearing evaluation; no authority rank.
- `CLAIM-TRANSITION.json` — append-only `DERIVED` / `PROMOTED` / `REVOKED` / `SUPERSEDED` transition entry bound to an external immutable registry's digest and trusted head.
- `CLAIM-TRANSITION-REGISTRY.json` — relying-party-supplied immutable transition history with whole-ledger digest, trusted head, exact predecessor membership, claim-ref continuity, and complete canonical chain reconstruction at every use.
- `CLAIM-REPRODUCTION-RECEIPT.json` — exact-subject reproduction bound to an external immutable independence admission, method/result bytes, and strict RFC3339 time; it can verify a fact but never promote policy authority.
- `REPRODUCTION-INDEPENDENCE-REGISTRY.json` — relying-party admission binding distinct producer/reproducer identities and trust domains, evidence, basis, admission issuer, immutable digests, and complete canonical semantic reconstruction at every use.
- `REASONING-SURFACE-OBSERVATION-RECEIPT.json` — single-transaction signed-in UI observation binding an externally required non-anonymous account, exact visible mode, session, evaluator-owned declarative input transform, admission question, completed response, strict-RFC3339 receipt and every non-null per-observation evidence time, post-response mode, and replay protection without claiming platform attestation.
- `SUPERVISION-VERDICT-ADMISSION.json` — single-use account-, admission-question-, response-digest-, and strict-RFC3339 issue/optional-admission-time binding between a reasoning-surface receipt and an admitted supervisory verdict; timestamp rejection precedes durable consumption.
- `RECEIPT-CONSUMPTION-EVENT.json` — append-only durable ledger event binding account, admission question, and response while preventing receipt/nonce/transaction replay across process restarts.
- `BROWSER-OPERATION-RECEIPT.json` — browser necessity, non-browser alternatives, session/transaction tab ownership, one-tab transient cap, protected owner tabs, cleanup, and strict-RFC3339 top-level receipt recording time; actions currently declare no timestamp field.
- `BROWSER-OWNERSHIP-REGISTRY.json` — immutable prior successful-OPEN proofs, with exact fields and complete canonical semantic reconstruction at every use, when a browser operation spans more than one receipt in the same session and transaction.
