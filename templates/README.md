...
- `AUTHORITY-SOURCE-REGISTRY.json` — relying-party-supplied immutable authority sources with per-source and whole-registry digests; runtime instances are factory-only, sealed, and exact-type checked so claims cannot self-authorize from embedded declarations or forged constructors.
...
- `CLAIM-TRANSITION-REGISTRY.json` — relying-party-supplied factory-only immutable transition history with whole-ledger digest, trusted head, exact predecessor membership, claim-ref continuity, and sealed runtime construction.
...
- `REPRODUCTION-INDEPENDENCE-REGISTRY.json` — relying-party factory-only admission binding distinct producer/reproducer identities and trust domains, evidence, basis, admission issuer, immutable digests, and sealed runtime construction.
...
- `BROWSER-OWNERSHIP-REGISTRY.json` — factory-only, sealed immutable prior successful-OPEN proofs when a browser operation spans more than one receipt in the same session and transaction.
