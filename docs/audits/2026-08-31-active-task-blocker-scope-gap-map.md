...
| Owner-outcome and task-local authority | Solved for identity/reconciliation | Added fixed authority precedence, current owner-source/correction projection, owner-stop override, and exact checkpoint path/ref/object/content/outcome binding | Reuse owner-source and owner-outcome fields; no second owner-authority system and no semantic owner interpretation in code |
...
| Wait admission | Reasoning-handoff polling was solved; general waits were not | Added `templates/WAIT-ADMISSION.json` plus exact blocker-unblock, owner-decision, live-handoff, and parsed horizon validation | Extends and delegates to the accepted handoff record without altering its reducer/resource accounting |
| Directive binding | Chat directive existed without blocker/wait identity | Added typed action class, `authorityContext`, affected-frontier state, blocker/revalidation/ambiguity identities, and fail-closed deterministic authorization | Compatible schema-v2 extension; placeholder remains parseable but is not executable authority |
...
Pro review `review-active-task-blocker-scope-1bc0e8e-pro-001` returned `ACCEPT_WITH_REVISION` with no owner decision. Its bounded correction cycle closed four gaps: non-waivable policy override prevention, fail-closed directive execution authority, current-owner/exact-checkpoint binding, and affected-frontier/wait enforcement. InnerSignal remains fixture-only and unmodified.
