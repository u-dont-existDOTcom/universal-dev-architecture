...
For multi-worker Codex operation with ChatGPT semantic supervision, load both `../patterns/codex-pro-supervision-mission-control.md` and the required owner correction `../patterns/codex-supervision-intelligence-routing-and-context-lifecycle.md`. The combined architecture separates Symphony execution, Linear work state, GitHub authority, deterministic evidence, optional Extra High repository retrieval, and high-intelligence Pro judgment. Maintain a logical supervision lane per task rather than one always-active Pro chat per worker; reserve Pro especially for therapy-answer semantics and AskRigor methodological/conclusion flaws, reuse related Pro chats with compact authority capsules and delta packets, and roll over only at explicit context, authority, contamination, or independence boundaries. Pro web supervisors must never depend on reliable GitHub access.
