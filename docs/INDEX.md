...
For multi-worker Codex operation with ChatGPT semantic supervision, load all four current Mission Control patterns:
...
Together they separate Symphony execution, Linear work state, GitHub authority, deterministic evidence, Extra High reasoning/repository review, Codex local execution, high-intelligence Pro judgment, and the immutable/versioned owner-outcome authority that every derived task contract and proposed terminal state must preserve.

For research that may span multiple sessions, workers, releases, future evidence, or scheduled surveillance, additionally load `../patterns/long-range-research-mission-supervision.md`, instantiate `../templates/RESEARCH-MISSION.json`, and give research workers `../templates/CURRENT-CODEX-WORKER-SUPERVISION-RESEARCH-ADDENDUM.md` alongside the shared bootstrap. The mission layer preserves one invariant owner/user purpose while permitting explicit versioned research-question amendments, epistemically independent work packages, evidence-frontier reporting, pause/resume/budget states, interim snapshots, living refreshes, independent consent/data authorities, and a separate public-release firewall. A completed work package or released version remains `SUBTASK_COMPLETE_PARENT_OPEN` unless the root mission closure comparator passes.
...
Before preserving an existing task contract, acceptance criterion, checkpoint, or completion boundary, recover the original owner request and compare the downstream contract directly against it. A derived contract may refine or decompose the owner outcome but may not weaken, omit, replace, or terminally bypass it without an explicit owner decision. Every checkpoint and packet must carry the owner-outcome identity, verbatim/normalized result, current gap, unmet outcomes, non-satisfying proxies, and terminal-comparator result. `READY_FOR_OWNER_REVIEW`, tests green, preservation PASS, reviewer approval, and similar supporting states are nonterminal by default while a required owner outcome remains unmet.

For the Mission Control pilot, also load the required owner-outcome addendum at `exec-plans/2026-08-30-mission-control-owner-outcome-terminal-integrity-addendum.md` and run `../evals/mission-control/contract-laundering-article-humanization-13.82.json`.
