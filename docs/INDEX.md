...
For substantial bespoke method/framework/architecture/metric/algorithm/taxonomy/protocol/evaluation/workflow invention that plausibly overlaps established knowledge, load `../patterns/research-before-reinvention.md` before further investment. When academic literature is material, that orchestration pattern routes to `../patterns/existing-work-scan-and-scholarly-discovery.md`; use a scholarly semantic search system such as SciSpace when available for terminology/literature discovery, then verify load-bearing claims against primary sources. Canonical repository-relative template: `templates/PRIOR-WORK-SCAN.md` (from this directory: `../templates/PRIOR-WORK-SCAN.md`). For recurring monitoring, load `../patterns/functional-neighborhood-discovery-for-monitoring.md`: a named target is an anchor into the owner’s underlying function/outcome, so every substantive pass should also look for equivalent/better alternatives, useful adjuncts, sibling/family candidates, different-mechanism solutions, community innovations/aliases, and displacement successors rather than waiting for a ban or searching only the original exact name.

For research that may span multiple sessions, workers, releases, future evidence, or scheduled surveillance, additionally load `../patterns/long-range-research-mission-supervision.md`, instantiate `../templates/RESEARCH-MISSION.json`, and give execution workers `../templates/CURRENT-CODEX-WORKER-SUPERVISION-RESEARCH-ADDENDUM.md` alongside the current shared supervision bootstrap. The mission layer preserves one invariant owner/user purpose while allowing explicit versioned research-question amendments, evidence-frontier reporting, independent work packages, pause/resume/budget states, interim snapshots, living refreshes, independent consent/data authorities, and a separate release firewall. A completed work package or released version remains subordinate to the root mission closure comparator.

For multi-worker Codex operation with ChatGPT semantic supervision, load all current Mission Control patterns. The first two are controlling wherever older language is ambiguous; the reasoning-selection pattern is a supplement and grants no new authority:
...
13. `../patterns/chatgpt-work-cloud-dispatch.md`
