...
**Continuation is goal-directed, not method-directed.** For substantive or iterative work, apply `patterns/outcome-advancement-and-strategy-efficacy.md`. Flat/regressing/unclear progress triggers diagnosis, not automatic replacement: check exposure, effect model/window, and source-bound bottleneck; refine only a supported implementation/preparation/pacing/delivery gap with a predeclared signal/review; stop or switch only for refusal, harm, infeasibility/resource exhaustion, specific mismatch, adequately tested contradiction/nonresponse, or a better supported alternative. Never invent fixed attempt counts for stochastic, skill-learning, or delayed strategies. If persistence is unclear, run the cheapest discriminating test. Sunk cost, green tests, or better execution do not justify a genuinely failing strategy. Continue automatically after supported continuation, refinement, or replacement unless a genuine owner/authority boundary blocks the next step.
