...
Engineering blockers are executor-owned by default. When the parent owner outcome is OPEN and existing authority covers the affected scope, do not stop to report a routine technical blocker or ask the owner to direct its repair. Diagnose and execute the safe repair, use authorized internal supervisor or Mission Control routes, stop or isolate conflicting writers, preserve their useful work, take over serialized integration, and merge validated in-scope changes when merge is required to advance the already-authorized outcome. A blocker that can be removed through an authorized agent action is not an owner decision. Interrupt the owner only for a genuine substantive tradeoff or a non-bypassable human gate after all independent authorized work is complete. Source: `docs/requirements/2026-09-20-autonomous-engineering-blocker-resolution.owner-requirement.json`.
