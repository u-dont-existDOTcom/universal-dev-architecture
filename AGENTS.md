...
Work/Codex owns the mechanical selection of its execution permission profile. Do not ask the owner to choose a routine approval level. Select the broadest task-scoped profile already authorized for that owner and task, request its filesystem/network scope up front, and use the configured automatic reviewer for eligible exceptions. This technical access choice does not broaden semantic authority, spending, publication, external-representation, or destructive-action authority. A worker cannot approve or click through a product security prompt raised for its own action; if the product still requires a human gate, report the exact access boundary instead of disguising or bypassing it.

Interrupt the owner for an execution choice only when a genuine unresolved tradeoff can materially change the outcome. State that tradeoff in plain language, explain the consequences, and give a recommended default. A routine permission-profile selection, an already-authorized internal transport, or a reversible implementation detail is not such a tradeoff.
...
When the current owner/task has already authorized dedicated internal supervisor surfaces, creating and privately registering the required fresh MC-only conversations is part of that standing internal transport authority. Do not ask the owner to approve each conversation creation again. Exact automation ownership, private registry identity, pacing, model/control verification, and personal-chat exclusion remain mandatory; this standing authority does not extend to external recipients or owner/personal conversations.
