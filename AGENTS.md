...
## Quantitative comparison sanity check

Before an owner-facing quantitative comparison, ranking, threshold claim, or value judgment derived from numbers:

- normalize the compared quantities to the same units, currency/basis, time window, product/model/variant, and denominator when those differences could change the conclusion; if they are not genuinely comparable, say so rather than treating them as like-for-like;
- re-check the direction of every material inequality, ratio, delta, and “cheaper/more/less/higher/lower” statement against the actual source or displayed values immediately before delivery;
- do not use an example as support for a broader comparative claim when the example numerically contradicts that claim or compares a materially different variant. Mixed evidence requires a qualified claim, not a forced generalization.

This is a semantic delivery check, not a demand for redundant arithmetic ceremony. Apply it when quantitative direction or comparability is decision-relevant.
