...
Before any browser or GUI action, perform a **browser-surface preflight**: discover the browser/computer-control surfaces actually available to the current Chat turn and distinguish them from shell/process access. A remote shell is not the user's graphical browser session unless that linkage is verified. Failure of shell launch helpers such as `xdg-open`, `open`, or `start` is not evidence that browser control is unavailable; never substitute a shell URL-launch attempt for current-turn browser-control discovery. Before asking the owner to open a URL, copy a code, click a control, or perform another browser step manually, use any authorized control surface for the automatable portion. For a genuine human-only authentication, consent, passkey, CAPTCHA, or similar gate, pre-position the exact page when possible, request only the irreducible gesture, then resume automatically after the gate clears.

Default browser automation to **headless mode**. Use a headed/visible browser only for visible browser/OS interaction, headed-only behavior, extension UI, native dialogs, window/focus behavior, WebAuthn/passkeys, visual debugging, or capability unavailable headlessly; preserve the reason in the task record when one exists.

Reuse the existing browser process/context/page/tab; navigate or reset it instead of churning tabs. Create fresh state only for isolation, parallelism, clean auth/storage, account separation, corrupt-state recovery, or behavior that depends on a fresh context. Close reusable state only when the task is finished or retention creates material resource, security, privacy, or contamination risk.

For managed ChatGPT browser automation, **New chat in the current verified reusable ChatGPT tab** means no new browser tab. Keep **one** managed tab in steady state; allow **two** only for transition/recovery; **three** is the hard ceiling and **fail closed before opening a fourth**. Open a **replacement** only after the current tab is irrecoverably unusable, then **close the superseded** tab. **Never fan out duplicate** tabs. Doctor/status telemetry must report `managedChatGptTabCount`; **clean** completed or superseded tabs back toward one. Do not retain bootstrap or pinned automation-owned tabs merely as history when durable capability/URL evidence exists.
