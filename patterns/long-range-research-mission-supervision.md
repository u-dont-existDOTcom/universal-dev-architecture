...
Status: current reusable extension to the shared reasoning/execution supervision architecture
  ...
  -> RESEARCH-MISSION.json authority
    -> versioned research questions
      -> independent work packages
        -> bounded research sessions / execution-worker tasks / provider operations
...
## 10. Reasoning and execution routing

Apply the current routing rules rather than hard-coding a model or provider into the mission pattern:

- `patterns/reasoning-selection.md` owns reasoning method and effort selection;
- `patterns/chat-work-execution-routing-threshold.md` keeps semantic/methodological reasoning in Chat and delegates only bounded execution residue;
- `patterns/work-model-and-effort-routing.md` owns the current Work model/effort ladder when Work is actually admitted;
- `patterns/existing-work-scan-and-scholarly-discovery.md` is the specialist discovery layer when academic literature is material.

Reserve the highest-intelligence reasoning surface for owner-value tradeoffs, consequential safety interpretation, conflicts among strong evidence sources, original novelty claims, public-release wording, sensitive-data/consent policy, material scope changes, and unresolved methodological judgments that could change conclusions.

Use ordinary reasoning/repository review for planning, literature and standards scans, source/synthesis comparison, audit reconstruction, and preparation of decision packets. Use execution workers for implementation, controlled acquisition, parsing, identity resolution, deduplication, deterministic calculations, schema validation, projections, and tests.

The selected reasoning model is never the canonical evidence store or mission controller. Mission authority remains the source-bound mission contract plus current owner/project authority.
