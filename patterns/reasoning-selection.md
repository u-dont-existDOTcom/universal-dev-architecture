...
Before an owner-facing quantitative comparison, perform a **comparison-sanity check**. Normalize decision-relevant units, currency/basis, time window, denominator, and product/model/variant so the quantities are genuinely comparable. Re-check the direction of every material inequality, ratio, delta, and “cheaper/more/less/higher/lower” statement against the actual source or displayed values. Do not use an example as support for a broader comparative claim when the example numerically contradicts that claim or compares a materially different variant; mixed evidence requires a qualified claim rather than a forced generalization.
...
The comparison-sanity check was added on 2026-09-21 after an owner correction exposed a directional quantitative-comparison failure: retrieved prices showed the cited competitor was more expensive, while the prose used that example to support a claim that Venice was not necessarily cheapest. The rule requires like-for-like normalization and a final direction check against the actual numbers.
