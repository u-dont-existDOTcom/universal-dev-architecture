...
The reference implementation is `scripts/git_exact_write.py`; its intended checks are clean worktree, exact branch/base/blob, payload identity, one-path staging, `git diff --cached --check`, local commit receipt, and restoration of the preimage on a pre-commit failure.
